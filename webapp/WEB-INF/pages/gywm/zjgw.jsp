...
<head>
<%
String _TITLE_ = "专家顾问 - 关于我们 - "+configureProvider.getProperty(SystemVariable.SITE_NAME);
String _KEYWORDS_ = "";
			...
			<div class="container fr">
				...
				<%
					...
					<dl>
						...
						<dd>
<%-- 						<%StringHelper.filterHTML(out, article.source);%> --%>
<%-- 						<%=TimestampParser.format(article.publishTime)%> --%>
							<p class="tit"><%
									StringHelper.filterHTML(out, article.title);
								%></p>
							<p class="h150"><%
									StringHelper
														.format(out,
																articleManage.getContent(article.id),
																fileStore);
								%></p>
						</dd>
					</dl>
				...
				<%
					Zjgw.rendPaging(out, results,
								controller.getPagingURI(request, Zjgw.class));
				%>
			</div>
		</div>
	</div>
