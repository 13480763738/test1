...
<head>
<%
String _TITLE_ = "合作机构 - 关于我们 - "+configureProvider.getProperty(SystemVariable.SITE_NAME);
String _KEYWORDS_ = "";
...
<div class="center_box white-bg pt30">
   	<div class="center">
       	<div class="partners_hf">
           	<div class="partners_tit_hf pl20 pr20">
		   <div class="new-about-xian"></div>
                     <h1 class="pb40">合作机构</h1>
                    </div>
           		<%
               ...
               <dl>
               	<dt><%
							if (!StringHelper.isEmpty(article.imageCode)) {
						%><img
							src="<%=fileStore.getURL(article.imageCode)%>" />
						<%
							}
						%></dt>
						
					<%
						String[] s = article.title.split(";");
					%>
						
                   <%if (s.length == 2) {if(s[1].equals("false")){StringHelper.filterHTML(out, s[0]);}else{%>
		                <a target="_blank" href="http://<%StringHelper.filterHTML(out, s[1]);%>">
		                <%StringHelper.filterHTML(out, s[0]);%></a>
	                <%}}%>
                   <dd class="txt"><%StringHelper.format(out,articleManage.getContent(article.id),fileStore);%></dd>
                   <div class="clear"></div>
               </dl>
