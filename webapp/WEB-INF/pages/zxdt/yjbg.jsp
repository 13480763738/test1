...
<%@page import="com.jiudian.p2p.front.servlets.Pviewer"%>
...
<head>
<%
	String _TITLE_ = "业绩报告 - "+configureProvider.getProperty(SystemVariable.SITE_NAME);
String _KEYWORDS_ = "";
...
<%
	CURRENT_CATEGORY = "YJBG";CURRENT_SUB_CATEGORY = "YJBG";
%>
<%@include file="/WEB-INF/include/gywm/left.jsp"%>
 <div class="center_box">
    	<div class="center">
        	<div class="new_list_box_hf">
            	<div class="new_list_tit_hf"><img src="/images/new_images/yjbg_tit.jpg"></div>
                	...
                	<ul>
                		<%
                			final int currentPage = IntegerParser.parse(request.getParameter("paging.current"));
                							...
                							PagingResult<PerformanceReport> results = reportManage.search(
                								new Paging() {
                							...
                							PerformanceReport[] reports = results.getItems();
                							if (reports != null && reports.length > 0) {
                								int index=1;
                								for (PerformanceReport report : reports) {
                		%>
		                <li><a target="_blank" href="<%=controller.getViewURI(request, Pviewer.class)%>?file=<%=controller.getPagingItemURI(request, Yjbg.class,report.id)%>">
		                <%StringHelper.filterHTML(out, report.title); %></a><span><%=DateParser.format(report.updateTime) %></span></li>
		                <%}}else{%>
		                <li>暂无报告。</li>
		                <%} %>
                    </ul>
