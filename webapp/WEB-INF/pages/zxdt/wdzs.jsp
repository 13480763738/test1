...
<head>
<%
String _TITLE_ = "最新动态 - 网贷知识 - "+configureProvider.getProperty(SystemVariable.SITE_NAME);
String _KEYWORDS_ = "";
...
<%@include file="/WEB-INF/include/gywm/left.jsp"%>
	<div class="center_box white-bg pt30">
    	<div class="center">
    	<div class="new_list_tit_hf pl20 pr20"><h1 style=" font-size:30px;">媒体报道</h1></div>
    	<div class="new_list_box_hf">
                <div class="new_list_hf pl20 pr20">
					...
					PagingResult<Article> results = articleManage.search(ArticleType.WDZS, new Paging() {public int getCurrentPage() {return currentPage;}public int getSize() {return 10;}});
					Article[] articles = results.getItems();
					if (articles != null && articles.length > 0) {
						for (Article article : articles) {
					%>
		            <li><a target="_blank" href="<%=controller.getPagingItemURI(request, Wdzs.class,article.id)%>?seo=<%=article.creater%>"><%StringHelper.filterHTML(out, article.title); %>
		            </a><span><%=DateParser.format(article.publishTime)%></span></li>
		            <%}}%>
		            </ul>
           			<%Mtbd.rendPaging(out, results,controller.getPagingURI(request, Wdzs.class)); %>
       	 		</div>
       	 		</div>
       	 </div>
        </div>
