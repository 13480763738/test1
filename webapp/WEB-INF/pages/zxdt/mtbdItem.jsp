...
Article  article = articleManage.get(IntegerParser.parse(request.getParameter("id")),ArticleType.MTBD);
if(article==null){
...
<head>
<%
String _TITLE_ = " - 最新动态 - 平台资讯 - "+configureProvider.getProperty(SystemVariable.SITE_NAME);
String _KEYWORDS_ = "";
...
<div class="center_box white-bg pt30">
        ...
        <div class="center">
        	<div class="new_details_bottom_hf">
       			<%Article  up = articleManage.getUp(IntegerParser.parse(request.getParameter("id")),ArticleType.MTBD);%>
       			<%Article  down = articleManage.getDown(IntegerParser.parse(request.getParameter("id")),ArticleType.MTBD);%>
       			<%if(up.id > 0){ %>
        		<span class="pre">上一篇：<a href="<%=controller.getPagingItemURI(request, Mtbd.class,up.id)%>?seo=<%=up.creater%>"><%StringHelper.filterHTML(out, up.title); %></a></span>
                <%} if(down.id >0){%>
                <span class="next">下一篇：<a href="<%=controller.getPagingItemURI(request, Mtbd.class,down.id)%>?seo=<%=down.creater%>"><%StringHelper.filterHTML(out, down.title); %></a></span>
                <%} %>
                 <div class="clear"></div>
            </div>
             <div class="clear"></div>
