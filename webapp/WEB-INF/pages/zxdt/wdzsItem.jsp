...
Article  article = articleManage.get(IntegerParser.parse(request.getParameter("id")),ArticleType.WDZS);
if(article==null){
...
<head>
<%
String _TITLE_ = " - 行业动态 - 网贷知识 - "+configureProvider.getProperty(SystemVariable.SITE_NAME);
String _KEYWORDS_ = "";
...
<%CURRENT_CATEGORY = "ZXDT";CURRENT_SUB_CATEGORY = "WDZS";%>
    	...
    	<div class="center_all_border">
            <div class="center">
                <div class="new_details_hf">
               		<dl class="pl20 pr20">
	                	<dt><span><%if(article!=null)StringHelper.filterHTML(out, article.title); %></span><em><%if(article!=null){out.print(DateParser.format(article.publishTime));} %></em>
	                	 <div class="clear"></div>
	                	</dt>
	                	 <dd class="target_ico">
	                	<%
                        	TitleWz[] titlewzlist1 = articleManage.getTitle(article.id); 
                        	if(titlewzlist1 != null && titlewzlist1.length >0){
                 		 %>
                         <span>标签：</span>
                         <em>
                         <%		
                        		
                        		for(TitleWz t : titlewzlist1){
                        %>
                        <a href="<%=controller.getViewForwardURI(request, Bqlb.class)%>?id=<%=t.id %>"><%StringHelper.filterHTML(out, t.name);%></a>
                        <%
                        		}
                        	}
                        %>
	                	...
	                	<dd> 
	                	<p><img src="<%=fileStore.getURL(article.imageCode) %>"/></p><br/>
		        	...
		        	<div class="new_details_bottom_hf">
		       			<%Article  up = articleManage.getUp(IntegerParser.parse(request.getParameter("id")),ArticleType.WDZS);%>
		       			<%Article  down = articleManage.getDown(IntegerParser.parse(request.getParameter("id")),ArticleType.WDZS);%>
		       			<%if(up.id > 0){ %>
		        		<span class="pre">上一篇：<a href="<%=controller.getPagingItemURI(request, Wdzs.class,up.id)%>?seo=<%=up.creater%>"><%StringHelper.filterHTML(out, up.title); %></a></span>
		                <%} if(down.id >0){%>
		                <span class="next">下一篇：<a href="<%=controller.getPagingItemURI(request, Wdzs.class,down.id)%>?seo=<%=down.creater%>"><%StringHelper.filterHTML(out, down.title); %></a></span>
		                <%} %>
		                 <div class="clear"></div>
		            </div>
		             <div class="clear"></div>
