...
Article  article = articleManage.get(IntegerParser.parse(request.getParameter("id")),ArticleType.WDHYZX);
if(article==null){
            ...
            <div class="center">
                <div class="new_details_hf">
        			...
        			<div class="new_details_bottom_hf">
		       			...
		       			<%Article  up = articleManage.getUp(IntegerParser.parse(request.getParameter("id")),ArticleType.WDHYZX);%>
		       			<%Article  down = articleManage.getDown(IntegerParser.parse(request.getParameter("id")),ArticleType.WDHYZX);%>		       		
		       			<%if(up.id > 0){ %>
		        		<span class="pre">上一篇：<a href="<%=controller.getPagingItemURI(request, Wdhyzx.class,up.id )%>?seo=<%=up.creater%>"><%StringHelper.filterHTML(out, up.title); %></a></span>
		                <%} if(down.id >0){%>
		                <span class="next">下一篇：<a href="<%=controller.getPagingItemURI(request, Wdhyzx.class,down.id )%>?seo=<%=down.creater%>"><%StringHelper.filterHTML(out, down.title); %></a></span>
						 <%} %>
		                 <div class="clear"></div>
		            </div>
		             <div class="clear"></div>
