...
Article  article = articleManage.get(IntegerParser.parse(request.getParameter("id")),ArticleType.HLWJRYJ);
if(article==null){
...
<head>
<%
String _TITLE_ = " - 互联网金融研究 - 平台资讯 - "+configureProvider.getProperty(SystemVariable.SITE_NAME);
String _KEYWORDS_ = "";
    	...
    	<div class="center_all_border">
            <div class="center">
                <div class="new_details_hf update">
               		<dl>
	                	<dt><span><%if(article!=null)StringHelper.filterHTML(out, article.title); %></span><em><%if(article!=null){out.print(Formater.formatDate(article.publishTime));} %></em></dt>
                        ...
                        %>
	                		<a href="<%=controller.getViewForwardURI(request, Bqlb.class)%>?id=<%=t.id %>&name=<%=t.name%>"><%StringHelper.filterHTML(out, t.name);%></a>
	                	<%
                        		}
                        	}
                        %>
		       		...
		       		<div class="new_details_bottom_hf update">
		      			<%Article  up = articleManage.getUp(IntegerParser.parse(request.getParameter("id")),ArticleType.HLWJRYJ);%>
		      			<%Article  down = articleManage.getDown(IntegerParser.parse(request.getParameter("id")),ArticleType.HLWJRYJ);%>
		      			<%if(up.id > 0){ %>
		       		<span class="pre">上一篇：<a href="<%=controller.getPagingItemURI(request, Hlwjryj.class,up.id)%>?seo=<%=up.creater%>"><%StringHelper.filterHTML(out, up.title); %></a></span>
		               <%} if(down.id >0){%>
		               <span class="next">下一篇：<a href="<%=controller.getPagingItemURI(request, Hlwjryj.class,down.id)%>?seo=<%=down.creater%>"><%StringHelper.filterHTML(out, down.title); %></a></span>
		               <%} %>
		           </div>
                </div>                
            </div>
