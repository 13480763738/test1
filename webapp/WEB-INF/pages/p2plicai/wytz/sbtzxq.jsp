...
<%@page import="com.jiudian.p2p.front.servlets.p2plicai.wytz.Zrjl"%>
<%@page import="com.jiudian.p2p.front.servlets.p2plicai.wytz.Zqxx"%>
<%@page import="com.jiudian.p2p.front.servlets.p2plicai.wytz.Hkjl"%>
...
<%
String _TITLE_ = "麒麟通宝 qltb.com";
String _KEYWORDS_ = "p2p网贷,网贷,网贷平台,p2p网贷平台,投资理财,互联网金融,100%本息担保, 麒麟通宝, qltb.com";
String _DESCRIPTION_ = "麒麟通宝 qltb.com - 供应链第一网络借贷平台";
%>
<title><%=_TITLE_ %></title>
...
<%-- 								<%if(RepaymentType.DQYCXFQ.equals(creditInfo.repaymentType)){ %>到期一次性偿还本金和利息。<%} %> --%>
                   ...
                   <%-- <%if(CreditCycle.AYHK.equals(creditInfo.creditCycle)){%>
                    <div class="list01_hf">月还利息（元）</div><div class="list01_hf"><span><%=creditInfo.yhkAmount%></span></div>
                    <%}%> --%>
					<div class="clear"></div>
            </li>
            <li>
		...
		<%if (creditInfo.status == CreditStatus.TBZ) {InvestInfoManage creditManagePayment = serviceSession.getService(InvestInfoManage.class);%>
			...
			%>
				...
				<%}else{ %>
					...
					<div class="fs18c73" style="background: #f2f3f5 none repeat scroll 0 0;height: 110px;text-align: center;line-height: 50px;">请
	                ...
	                <div class="daojishi" style="width:190px; height:30px; margin:0 auto; text-align:center;">
	        	<div class="dao_time">
	...
	<!--             	</em><span>0</span><em>天 -->
				...
				<%}}else if (creditInfo.status.equals(CreditStatus.YFK)) {%>
					...
					<div class="manbiao"> 
						<p class="title">还款中</p>
						<div class="m_bottom">
							<p class="bx" style="padding-left:0px;">待还本息（元）   <span> ¥ <%=Formater.formatAmount(creditInfo.toRepaymentAmount)%></span></p>
...
<div class="cgcz_box2 tc_qrzf" style="width:720px; margin-top:-140px; margin-left:-315px;display: none;">
	...
	<br/><input type="checkbox" checked="checked" value="1" class="m_cb selected" id="isread" name="isread" style="display:inline-block;margin:5px 3px 0 0; float:left;">
	<label style="font-size: 12px;display:inline-block;float:left; margin-top:5px;">我已阅读并同意</label>
