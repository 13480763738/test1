...
<%
	...
	URLParameter parameter = new URLParameter(request,controller.getViewURI(request, com.jiudian.p2p.front.servlets.p2plicai.qqd.Index.class),multiSelect,MULTISELECT_KEY,TYPE_KEY,TERM_KEY,LEVEL_KEY);
	...
	new InvestQuery() {
		...
		public CreditTerm[] getTerm() {
			...
			CreditTerm[] levels = new CreditTerm[values.length];
			for(int i=0;i<values.length;i++){
				levels[i] = EnumParser.parse(CreditTerm.class,values[i]);
			}
			return levels;
		}
		public ProductType getProductType() {
			return EnumParser.parse(ProductType.class, ProductType.QQD.name());
		}
	}, new Paging() {
...
<div class="content_part2 container" style=" background:none; height:900px;">
     <p class="f18 dark_gray pl20 pb20">旗房贷列表</p>
     <div class="biaodi">
        	       ...
        	       <td name="tbname3bt"><table align="center" cellspacing="0" width="100%">
					 ...
					 %>
        	           <tr class="all_bjs">
        	             ...
        	             <%i++;%>
        	             <td height="64" name="tbname<%=i%>jd" <%if((creditInfo.status==CreditStatus.TBZ && creditInfo.fbTime.after(new Date()))){%>style="display:none;"<%}%>><div class="pl30"><span class="ui-list-field"><strong class="ui-progressbar-mid ui-progressbar-mid-<%=creditInfo.progress%>">
