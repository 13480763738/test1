...
<%@page import="com.jiudian.p2p.front.servlets.p2plicai.kxt.Bdxq"%>
...
<%
	...
	URLParameter parameter = new URLParameter(request,controller.getViewURI(request, com.jiudian.p2p.front.servlets.p2plicai.qfd.Index.class),multiSelect,MULTISELECT_KEY,TYPE_KEY,TERM_KEY,LEVEL_KEY);
	...
	return EnumParser.parse(ProductType.class, ProductType.QFD.name());
		}
	}, new Paging() {
...
<div class="content_part2 container" style=" background:none; height:900px; margin-top:42px;">
     ...
     <div class="biaodi">
        ...
        <div class="biaodi_list">
        	         ...
        	         <tbody>
        	           ...
        	           <tr class="all_bjs">
        	             <td width="400" height="64">
        	               ...
        	               %><p><span class="ico <%if(creditInfo.companyStatus!=null && creditInfo.companyStatus.equals(CompanyStatus.SYQYZ)){%>ico01<%}%>"></span><span class="ico ico03"></span></p><%
        	               	}
        	               ...
        	               %><p><span class="ico <%if(creditInfo.companyStatus!=null && creditInfo.companyStatus.equals(CompanyStatus.SYQYZ)){%>ico01<%}%>"></span><span class="ico ico02"></span></p><%
        	               	}
        	               %>
        	               <a class="title" title="<%StringHelper.filterHTML(out,creditInfo.title);%>" href="<%=controller.getPagingItemURI(request, Sbtzxq.class,creditInfo.id)%>"><%
        	             ...
        	             <td name="tbname<%=i%>bt" <%if((creditInfo.status==CreditStatus.TBZ && creditInfo.fbTime.after(new Date()))){%>style="display:none;"<%}%>>
								...
								%><a href="javascript:void(0)" class="toubiao2 btn01">已满标</a>
								<%
									}else if(creditInfo.status==CreditStatus.YFK){
								%><a href="javascript:void(0)" class="toubiao2 btn01">还款中</a>
								<%
									}else if(creditInfo.status==CreditStatus.YJQ){
								%><a href="javascript:void(0)" class="toubiao2 btn01">已结清</a>
								<%
									}
								%>
                       </td>
             ...
             <div class="clear"></div>
             <%
             	AbstractFinancingServlet.rendPaging(out, result,controller.getPagingURI(request, com.jiudian.p2p.front.servlets.p2plicai.qfd.Index.class),parameter.getQueryString());
             %>
        </div> 
    </div>
</div>          
