...
<%@include file="/WEB-INF/include/upload.jsp" %>
	<div class="popup_tips" >
			<div class="popup_tips">
			    ...
			    <div class="con clearfix">
			    	<div class="bigtit">购车认证</div>
			        <div class="info_top">购车认证是证明借入者资产及还款能力的重要凭证之一。</div>
			        <div class="mt10">
			           <div class="smalltit">认证说明：</div>
			           <p>1、	请上传您所购买<i class="d_red">车辆行驶证</i>原件的照片。 <a href="<%=controller.getViewURI(request, Gcsl.class) %>" target="_blank" class="blue ml15">查看示例</a></p>
						<p>2、	请上传您和您购买车辆的<i class="d_red">合影（照片须露出车牌号码）</i>。 <a href="<%=controller.getViewURI(request, Gchysl.class) %>" target="_blank" class="blue ml15">查看示例</a></p>
						<p>3、	请确认您上传的资料是清晰的、未经修改的数码照片或扫描图片。</p>
						
			           <p><i class="f16 gray33">认证有效期：</i><i class="d_red">永久</i></p>
			        </div>
			        <div class="mt10">
			            <p><i class="d_red mr5">*</i><i class="f16 gray33">上传购车认证：</i></p>
				            <form action="<%=controller.getURI(request, Car.class)%>" method="post" enctype="multipart/form-data">
	...
	$(function() {
		$("#uploader").plupload({
			...
			url : '<%=controller.getURI(request, Car.class)%>',
			max_file_size : '4mb',
			chunk_size : '4mb',			
			filters : [ {
				title : "Image files",
