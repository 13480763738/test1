...
<%@page import="com.jiudian.p2p.front.servlets.p2pdaikuan.xjd.Showcredit"%>
<%@page import="com.jiudian.p2p.front.servlets.Term"%>
<%@page import="com.jiudian.p2p.front.servlets.p2pdaikuan.xjd.Basemessage"%>
<%@page import="com.jiudian.p2p.front.servlets.p2pdaikuan.xjd.Index"%>
<%@page import="com.jiudian.p2p.front.servlets.p2pdaikuan.xjd.Checkmessage"%>
...
<%@page import="com.jiudian.p2p.front.servlets.p2pdaikuan.xjd.AddCredit"%>
...
<body>
				...
				<div class="loan_con ">
					<ul>
						...
						<li id="mst2" >
							...
							<div class="info"><span>¥</span><span id="ms">0.00</span>
							</div>
						</li>
						<li id="mjt">
						...
						<li>
							...
							<table border="0" cellspacing="0" class="ratetable">
								<tbody>
									...
									<tr>
										<td width="15%">费率</td>
										<td><%=Formater.formatRate(Double.parseDouble(configureProvider.getProperty(SystemVariable.SUCCESS_BMONEY_RATE_AA)))%></td>
										<td><%=Formater.formatRate(Double.parseDouble(configureProvider.getProperty(SystemVariable.SUCCESS_BMONEY_RATE_A)))%></td>
										<td><%=Formater.formatRate(Double.parseDouble(configureProvider.getProperty(SystemVariable.SUCCESS_BMONEY_RATE_B)))%></td>
										<td><%=Formater.formatRate(Double.parseDouble(configureProvider.getProperty(SystemVariable.SUCCESS_BMONEY_RATE_C)))%></td>
										<td><%=Formater.formatRate(Double.parseDouble(configureProvider.getProperty(SystemVariable.SUCCESS_BMONEY_RATE_D)))%></td>
										<td><%=Formater.formatRate(Double.parseDouble(configureProvider.getProperty(SystemVariable.SUCCESS_BMONEY_RATE_E)))%></td>
										<td><%=Formater.formatRate(Double.parseDouble(configureProvider.getProperty(SystemVariable.SUCCESS_BMONEY_RATE_HR)))%></td>
									</tr>
									<tr>
										<td width="15%">实际费用</td>
	...
	$(".jejs").change( function() {
	  ...
	  $crehr.html((parseInt(money)*parseFloat($crehr.attr("fl"))*size).toFixed(2));
	   ...
	   //月还本息
	   var hkfs = $("select[name='getType']").val();
	   //不扣首期利息
