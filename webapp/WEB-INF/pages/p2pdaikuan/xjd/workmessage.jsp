...
<body>
	...
	<form action="<%=controller.getURI(request, UpdateWorkmessage.class)%>"
		...
		<div class="contain clearfix">
			...
			<div class="about clearfix">
				...
				<div class="container fr">
					<div class="public_tit">工作信息填写</div>
					<div class="loan_con clearfix">
						<ul>
							<li>
								...
								<div class="info">
									<select name="status" class="sel" style="display: none;">
										<option value="<%=WorkStatus.GXJC.name()%>">工薪阶层</option>
									</select>
									工薪阶层
								</div>
							</li>
							<li>
								...
								<div class="info">
									...
									<%
										AttestationState state = lmoneyManage.getAttentiontype(AttestationType.GZRZ);
							...
							<li>
								<div class="tit">
									<i class="red mr5">*</i>职 位：
								</div>
								<div class="info">
									<input name="post" type="text"
										value="<%StringHelper.filterHTML(out, data.Post);%>"
							...
							<li>
								...
								<div class="info">
									<input name="address" type="text"
										...
										class="put01 required max-lenght-30" />
									<p tip>不超过30个字</p>
									<p errortip class="" style="display: none"></p>
								</div>
							</li>
							<li>
								...
								<div class="info">
									<select name="type" class="sel">
										<option
											<%if(!StringHelper.isEmpty(data.Type) && data.Type.equals("国家机关")){%>
											selected="selected" <%}%> value="国家机关">国家机关</option>
										<option
											<%if(!StringHelper.isEmpty(data.Type) && data.Type.equals("事业单位")){%>
											selected="selected" <%}%> value="事业单位">事业单位</option>
										<option
											<%if(!StringHelper.isEmpty(data.Type) && data.Type.equals("央企（包括下级单位）")){%>
											selected="selected" <%}%> value="央企（包括下级单位）">央企（包括下级单位）</option>
										<option
											<%if(!StringHelper.isEmpty(data.Type) && data.Type.equals("地方国资委直属企业")){%>
											selected="selected" <%}%> value="地方国资委直属企业">地方国资委直属企业</option>
										<option
											<%if(!StringHelper.isEmpty(data.Type) && data.Type.equals("世界500强（包括合资企业及下级单位）")){%>
											selected="selected" <%}%> value="世界500强（包括合资企业及下级单位）">世界500强（包括合资企业及下级单位）</option>
										<option
											<%if(!StringHelper.isEmpty(data.Type) && data.Type.equals("外资企业（包括合资企业）")){%>
											selected="selected" <%}%> value="外资企业（包括合资企业）">外资企业（包括合资企业）</option>
										<option
											<%if(!StringHelper.isEmpty(data.Type) && data.Type.equals("一般上市公司（包括国外上市）")){%>
											selected="selected" <%}%> value="一般上市公司（包括国外上市）">一般上市公司（包括国外上市）</option>
										<option
											<%if(!StringHelper.isEmpty(data.Type) && data.Type.equals("一般民营企业")){%>
											selected="selected" <%}%> value="一般民营企业">一般民营企业</option>
										<option
											<%if(!StringHelper.isEmpty(data.Type) && data.Type.equals("个体经营者")){%>
											selected="selected" <%}%> value="个体经营者">个体经营者</option>
										<option
											<%if(!StringHelper.isEmpty(data.Type) && data.Profession.equals("其他")){%>
							...
							<li>
								...
								<div class="info">
									<select name="profession" class="sel">
										<option
											<%if(!StringHelper.isEmpty(data.Profession) && data.Profession.equals("制造业")){%>
											selected="selected" <%}%> value="制造业">制造业</option>
										<option
											<%if(!StringHelper.isEmpty(data.Profession) && data.Profession.equals("IT")){%>
											selected="selected" <%}%> value="IT">IT</option>
										<option
											<%if(!StringHelper.isEmpty(data.Profession) && data.Profession.equals("政府机关")){%>
											selected="selected" <%}%> value="政府机关">政府机关</option>
										<option
											<%if(!StringHelper.isEmpty(data.Profession) && data.Profession.equals("媒体/广告")){%>
											selected="selected" <%}%> value="媒体/广告">媒体/广告</option>
										<option
											<%if(!StringHelper.isEmpty(data.Profession) && data.Profession.equals("零售/批发")){%>
											selected="selected" <%}%> value="零售/批发">零售/批发</option>
										<option
											<%if(!StringHelper.isEmpty(data.Profession) && data.Profession.equals("教育/培训")){%>
											selected="selected" <%}%> value="教育/培训">教育/培训</option>
										<option
											<%if(!StringHelper.isEmpty(data.Profession) && data.Profession.equals("公共事业")){%>
											selected="selected" <%}%> value="公共事业">公共事业</option>
										<option
											<%if(!StringHelper.isEmpty(data.Profession) && data.Profession.equals("交通运输业")){%>
											selected="selected" <%}%> value="交通运输业">交通运输业</option>
										<option
											<%if(!StringHelper.isEmpty(data.Profession) && data.Profession.equals("房地产业")){%>
											selected="selected" <%}%> value="房地产业">房地产业</option>
										<option
											<%if(!StringHelper.isEmpty(data.Profession) && data.Profession.equals("能源业")){%>
											selected="selected" <%}%> value="能源业">能源业</option>
										<option
											<%if(!StringHelper.isEmpty(data.Profession) && data.Profession.equals("金融/法律")){%>
											selected="selected" <%}%> value="金融/法律">金融/法律</option>
										<option
											<%if(!StringHelper.isEmpty(data.Profession) && data.Profession.equals("餐饮/旅馆业")){%>
											selected="selected" <%}%> value="餐饮/旅馆业">餐饮/旅馆业</option>
										<option
											<%if(!StringHelper.isEmpty(data.Profession) && data.Profession.equals("医疗/卫生/保健")){%>
											selected="selected" <%}%> value="医疗/卫生/保健">医疗/卫生/保健</option>
										<option
											<%if(!StringHelper.isEmpty(data.Profession) && data.Profession.equals("建筑工程")){%>
											selected="selected" <%}%> value="建筑工程">建筑工程</option>
										<option
											<%if(!StringHelper.isEmpty(data.Profession) && data.Profession.equals("农业")){%>
											selected="selected" <%}%> value="农业">农业</option>
										<option
											<%if(!StringHelper.isEmpty(data.Profession) && data.Profession.equals("娱乐服务业")){%>
											selected="selected" <%}%> value="娱乐服务业">娱乐服务业</option>
										<option
											<%if(!StringHelper.isEmpty(data.Profession) && data.Profession.equals("体育/艺术")){%>
											selected="selected" <%}%> value="体育/艺术">体育/艺术</option>
										<option
											<%if(!StringHelper.isEmpty(data.Profession) && data.Profession.equals("公益组织")){%>
											selected="selected" <%}%> value="公益组织">公益组织</option>
										<option
											<%if(!StringHelper.isEmpty(data.Profession) && data.Profession.equals("其他")){%>
											selected="selected" <%}%> value="其他">其他</option>
									</select>
								</div>
							</li>
							<li>
								...
								<div class="info">
									<select name="scale" class="sel">
										<option
											<%if(!StringHelper.isEmpty(data.Scale) && data.Scale.equals("10")){%>
											selected="selected" <%}%> value="10">10人以下</option>
										<option
											<%if(!StringHelper.isEmpty(data.Scale) && data.Scale.equals("10-100")){%>
											selected="selected" <%}%> value="10-100">10-100人</option>
										<option
											<%if(!StringHelper.isEmpty(data.Scale) && data.Scale.equals("100-500")){%>
											selected="selected" <%}%> value="100-500">100-500人</option>
										<option
											<%if(!StringHelper.isEmpty(data.Scale) && data.Scale.equals("500")){%>
							...
							<li>
								<div class="tit">
									<i class="red mr5">*</i>在现单位工作年限：
								</div>
								<div class="info">
									<select name="gznew" class="sel">
										<option
											<%if(!StringHelper.isEmpty(data.Gznew) && data.Gznew.equals("1")){%>
											selected="selected" <%}%> value="1">1年（含以下）</option>
										<option
											<%if(!StringHelper.isEmpty(data.Gznew) && data.Gznew.equals("1-3")){%>
											selected="selected" <%}%> value="1-3">1-3年（含）</option>
										<option
											<%if(!StringHelper.isEmpty(data.Gznew) && data.Gznew.equals("3-5")){%>
											selected="selected" <%}%> value="3-5">3-5年（含）</option>
										<option
											<%if(!StringHelper.isEmpty(data.Gznew) && data.Gznew.equals("5")){%>
							...
							<li>
								...
								<div class="info">
									<input name="sta" type="text" class="put02 required isint  max-length-4"
										...
										if(data.Cellnumber.split("-").length>0){
											String s[] = data.Cellnumber.split("-");
											sta=s[0];
											end=s[1];}
										}%><%=sta%>" />&nbsp;<span style="float:left;">-&nbsp;</span>&nbsp;&nbsp;<input
										name="end" type="text" value="<%=end%>"
										class="put03 required isint required max-length-8" />
									<p tip>必须为数字</p>
									<p errortip class="" style="display: none"></p>
								</div>
							</li>
