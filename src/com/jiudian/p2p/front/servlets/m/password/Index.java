package com.jiudian.p2p.front.servlets.m.password;
...
import com.jiudian.p2p.front.servlets.password.AbstractPasswordServlet;
...
public class Index extends AbstractPasswordServlet {
	...
	protected void processPost(HttpServletRequest request,
		...
		if ("phone".equals(type)) {
			...
			if (id <= 0) {
				request.setAttribute("PHONE_ERROR", "手机号码未注册");
				forward(request, response,
						getController().getViewURI(request, Index.class));
				return;
			}
	...
	protected void onThrowable(HttpServletRequest request,
		...
		if(throwable instanceof AuthenticationException){
			String type = request.getParameter("type");
			if("phone".equals(type)){
				request.setAttribute("PHONE_ERROR", "验证码错误");
				forward(request, response,
						getController().getViewURI(request, Index.class));
			}
		}
