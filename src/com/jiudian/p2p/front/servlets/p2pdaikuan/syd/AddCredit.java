...
import com.jiudian.p2p.front.servlets.p2pdaikuan.xjd.Index;
...
public class AddCredit extends AbstractCreditServlet {
	...
	protected void processPost(final HttpServletRequest request,
		...
		if(lmoneyManage.isyq()){
			//存在逾期不能借款
			sendRedirect(request, response, getController().getViewURI(request, Index.class)+"?erd=4");
		}
		
		
		int oid = lm.id;
		if(oid !=0){
			Lmoney l = lmoneyManage.getLmoney(oid, CreditType.SYD.name());
			if(l != null){
				if(oid!=0 && l.id==0){
					sendRedirect(request, response, getController().getViewURI(request, Index.class)+"?erd=1");
					return ;
				}
	...
	protected void onThrowable(HttpServletRequest request,
		...
		if (throwable instanceof AuthenticationException) {
			...
			getController().forward(request, response, getController().getViewURI(request, Syd.class));
		}
	}
}
